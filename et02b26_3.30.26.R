...
```{r}
featurecounts_txt <- readr::read_tsv("consensuscoverage_countmatrix.txt")
...
```{r}
annotations_txt <- read.table("annotatedbed5.txt", sep = "\t", header = FALSE)
...
#readr::write_tsv(wholedf, "et02b26_consensuscoverage_counts_annotations.txt")
```
...
```{r}
IFNL2peaks <- wholedf %>% filter(., Genes %in% "IFNL2")
IFNL2peaks_1 <- IFNL2peaks %>%
        pivot_longer(!c(Genes,Interval, tss, gene_id, transcript_type, stop, start, chromosome), 
                     names_to = "Replicate", values_to = "Counts") %>%
        mutate(clone = gsub("(\\w+)_(\\w+)", "\\1", Replicate))
IFNL2peaks_1
...
logtransformedcounts <- log2(IFNL2peaks_1$Counts)

IFNL2peaks_2 <- cbind(IFNL2peaks_1, logtransformedcounts)
IFNL2peaks_2$clone <- factor(IFNL2peaks_2$clone, levels = c("et10", "et3", "et1", "wt", "et7", "et8", "et2", "et9"))

# rough plotting of MAVS peaks for each interval in each clone
ggplot(IFNL2peaks_2, aes(x = Interval, y = Counts, color = clone, fill = clone, group = clone))+
        geom_jitter(position = position_jitterdodge(jitter.width = 0.2, 
                                                    dodge.width = 2), size = 2)

IFNL2peaks_2$logtransformedcounts
```

```{r}
IFNL1peaks <- wholedf %>% filter(., Genes %in% "IFNL1")
IFNL1peaks_1 <- IFNL1peaks %>%
        pivot_longer(!c(Genes,Interval, tss, gene_id, transcript_type, stop, start, chromosome), 
                     names_to = "Replicate", values_to = "Counts") %>%
        mutate(clone = gsub("(\\w+)_(\\w+)", "\\1", Replicate))
IFNL1peaks_1
...
logtransformedcounts <- log2(IFNL1peaks_1$Counts)

IFNL1peaks_2 <- cbind(IFNL1peaks_1, logtransformedcounts)
IFNL1peaks_2$clone <- factor(IFNL1peaks_2$clone, levels = c("et10", "et3", "et1", "wt", "et7", "et8", "et2", "et9"))

# rough plotting of MAVS peaks for each interval in each clone
ggplot(IFNL1peaks_2, aes(x = Interval, y = Counts, color = clone, fill = clone, group = clone))+
        geom_jitter(position = position_jitterdodge(jitter.width = 0.2, 
                                                    dodge.width = 2), size = 2)
IFNL1peaks_2$logtransformedcounts
```

```{r}
IFNL3peaks <- wholedf %>% filter(., Genes %in% "IFNL3")
IFNL3peaks_1 <- IFNL3peaks %>%
        pivot_longer(!c(Genes,Interval, tss, gene_id, transcript_type, stop, start, chromosome), 
                     names_to = "Replicate", values_to = "Counts") %>%
        mutate(clone = gsub("(\\w+)_(\\w+)", "\\1", Replicate))
IFNL3peaks_1
...
logtransformedcounts <- log2(IFNL3peaks_1$Counts)

IFNL3peaks_2 <- cbind(IFNL3peaks_1, logtransformedcounts)
IFNL3peaks_2$clone <- factor(IFNL3peaks_2$clone, levels = c("et10", "et3", "et1", "wt", "et7", "et8", "et2", "et9"))

# rough plotting of MAVS peaks for each interval in each clone
ggplot(IFNL3peaks_2, aes(x = Interval, y = Counts, color = clone, fill = clone, group = clone))+
        geom_jitter(position = position_jitterdodge(jitter.width = 0.2, 
                                                    dodge.width = 2), size = 2)

IFNL3peaks_2$logtransformedcounts
```

```{r}
IFNB1peaks <- wholedf %>% filter(., Genes %in% "IFNB1")
IFNB1peaks_1 <- IFNB1peaks %>%
        pivot_longer(!c(Genes,Interval, tss, gene_id, transcript_type, stop, start, chromosome), 
                     names_to = "Replicate", values_to = "Counts") %>%
        mutate(clone = gsub("(\\w+)_(\\w+)", "\\1", Replicate))
IFNB1peaks_1
...
logtransformedcounts <- log2(IFNB1peaks_1$Counts)

IFNB1peaks_2 <- cbind(IFNB1peaks_1, logtransformedcounts)
IFNB1peaks_2$clone <- factor(IFNB1peaks_2$clone, levels = c("et10", "et3", "et1", "wt", "et7", "et8", "et2", "et9"))

# rough plotting of MAVS peaks for each interval in each clone
ggplot(IFNB1peaks_2, aes(x = Interval, y = Counts, color = clone, fill = clone, group = clone))+
        geom_jitter(position = position_jitterdodge(jitter.width = 0.2, 
                                                    dodge.width = 2), size = 2)

IFNB1peaks_2$logtransformedcounts
```

```{r}
IFNA2peaks <- wholedf %>% filter(., Genes %in% "IFNA2")
IFNA2peaks_1 <- IFNA2peaks %>%
        pivot_longer(!c(Genes,Interval, tss, gene_id, transcript_type, stop, start, chromosome), 
                     names_to = "Replicate", values_to = "Counts") %>%
        mutate(clone = gsub("(\\w+)_(\\w+)", "\\1", Replicate))
IFNA2peaks_1
...
logtransformedcounts <- log2(IFNA2peaks_1$Counts)

IFNA2peaks_2 <- cbind(IFNA2peaks_1, logtransformedcounts)
IFNA2peaks_2$clone <- factor(IFNA2peaks_2$clone, levels = c("et10", "et3", "et1", "wt", "et7", "et8", "et2", "et9"))

# rough plotting of MAVS peaks for each interval in each clone
ggplot(IFNA2peaks_2, aes(x = Interval, y = Counts, color = clone, fill = clone, group = clone))+
        geom_jitter(position = position_jitterdodge(jitter.width = 0.2, 
                                                    dodge.width = 2), size = 2)

IFNA2peaks_2$logtransformedcounts
```

```{r}
IFNA5peaks <- wholedf %>% filter(., Genes %in% "IFNA5")
IFNA5peaks_1 <- IFNA5peaks %>%
        pivot_longer(!c(Genes,Interval, tss, gene_id, transcript_type, stop, start, chromosome), 
                     names_to = "Replicate", values_to = "Counts") %>%
        mutate(clone = gsub("(\\w+)_(\\w+)", "\\1", Replicate))
IFNA5peaks_1
...
logtransformedcounts <- log2(IFNA5peaks_1$Counts)

IFNA5peaks_2 <- cbind(IFNA5peaks_1, logtransformedcounts)
IFNA5peaks_2$clone <- factor(IFNA5peaks_2$clone, levels = c("et10", "et3", "et1", "wt", "et7", "et8", "et2", "et9"))

# rough plotting of MAVS peaks for each interval in each clone
ggplot(IFNA5peaks_2, aes(x = Interval, y = Counts, color = clone, fill = clone, group = clone))+
        geom_jitter(position = position_jitterdodge(jitter.width = 0.2, 
                                                    dodge.width = 2), size = 2)
...
```{r}
IFNA8peaks <- wholedf %>% filter(., Genes %in% "IFNA8")
IFNA8peaks_1 <- IFNA8peaks %>%
        pivot_longer(!c(Genes,Interval, tss, gene_id, transcript_type, stop, start, chromosome), 
                     names_to = "Replicate", values_to = "Counts") %>%
        mutate(clone = gsub("(\\w+)_(\\w+)", "\\1", Replicate))
IFNA8peaks_1
...
logtransformedcounts <- log2(IFNA8peaks_1$Counts)

IFNA8peaks_2 <- cbind(IFNA8peaks_1, logtransformedcounts)
IFNA8peaks_2$clone <- factor(IFNA8peaks_2$clone, levels = c("et10", "et3", "et1", "wt", "et7", "et8", "et2", "et9"))

# rough plotting of MAVS peaks for each interval in each clone
ggplot(IFNA8peaks_2, aes(x = Interval, y = Counts, color = clone, fill = clone, group = clone))+
        geom_jitter(position = position_jitterdodge(jitter.width = 0.2, 
                                                    dodge.width = 2), size = 2)
```
